...
public class DstProtocolConformanceTests
{
    ...
    public void Encode_ControlBytes_CorrectValues()
    {
        ...
        // Test END - must use EncodeEnd() directly (EncodeMovement rejects StitchEnd)
        var (eb1, eb2, eb3) = DstMovementEncoder.EncodeEnd();
        eb1.Should().Be(0xF3);
        eb2.Should().Be(0x00);
        eb3.Should().Be(0x00);
    }
    ...
    #region 13b: Independent Reference Golden Vectors (Pyembroidery/Libembroidery)

    /// <summary>
    /// Golden vectors from pyembroidery (libembroidery) reference implementation.
    /// These are INDEPENDENT verification bytes - NOT generated by our encoder.
    /// 
    /// Reference: pyembroidery.DstWriter.encode_record() with Tajima spec:
    /// - Y axis is NEGATED before encoding (y = -y)
    /// - Balanced ternary magnitudes: 1, 3, 9, 27, 81
    /// - Bit layout per encode_record(): byte0=Y bits 7,6,2,1, X bits 2,3,0,1; byte1=Y bits 5,4,7,6, X bits 2,3,0,1; byte2=control bits 7,6, Y bits 5,4, X bits 3,2
    /// - Control: Normal=0x03 (sync), Jump=0x83, ColorChange=0xC3, End=0xF3
    /// 
    /// Format: (deltaX, deltaY, controlName, expectedByte1, expectedByte2, expectedByte3)
    /// All coordinates in DST units (1 unit = 0.1mm = 100 microns)
    /// </summary>
    public static readonly object[][] GoldenVectors =
    {
        // Normal stitches (control = 0x00, byte3 = 0x03 with sync bits)
        new object[] { 0, 0, "Normal", (byte)0x00, (byte)0x00, (byte)0x03 },
        new object[] { 1, 0, "Normal", (byte)0x01, (byte)0x00, (byte)0x03 },
        new object[] { -1, 0, "Normal", (byte)0x02, (byte)0x00, (byte)0x03 },
        new object[] { 0, 1, "Normal", (byte)0x40, (byte)0x00, (byte)0x03 },   // Y positive -> byte1 bit 6
        new object[] { 0, -1, "Normal", (byte)0x80, (byte)0x00, (byte)0x03 },  // Y negative -> byte1 bit 7
        new object[] { 3, 0, "Normal", (byte)0x00, (byte)0x01, (byte)0x03 },
        new object[] { -3, 0, "Normal", (byte)0x00, (byte)0x02, (byte)0x03 },
        new object[] { 0, 3, "Normal", (byte)0x00, (byte)0x40, (byte)0x03 },   // Y+=3 -> byte2 bit 6
        new object[] { 0, -3, "Normal", (byte)0x00, (byte)0x80, (byte)0x03 },  // Y-=3 -> byte2 bit 7
        new object[] { 9, 0, "Normal", (byte)0x04, (byte)0x00, (byte)0x03 },
        new object[] { -9, 0, "Normal", (byte)0x08, (byte)0x00, (byte)0x03 },
        new object[] { 0, 9, "Normal", (byte)0x10, (byte)0x00, (byte)0x03 },   // Y+=9 -> byte1 bit 4
        new object[] { 0, -9, "Normal", (byte)0x20, (byte)0x00, (byte)0x03 },  // Y-=9 -> byte1 bit 5
        new object[] { 27, 0, "Normal", (byte)0x00, (byte)0x04, (byte)0x03 },
        new object[] { -27, 0, "Normal", (byte)0x00, (byte)0x08, (byte)0x03 },
        new object[] { 0, 27, "Normal", (byte)0x00, (byte)0x10, (byte)0x03 },  // Y+=27 -> byte2 bit 4
        new object[] { 0, -27, "Normal", (byte)0x00, (byte)0x20, (byte)0x03 }, // Y-=27 -> byte2 bit 5
        new object[] { 81, 0, "Normal", (byte)0x00, (byte)0x00, (byte)0x07 },  // X+=81 -> byte3 bit 2 + sync
        new object[] { -81, 0, "Normal", (byte)0x00, (byte)0x00, (byte)0x0B }, // X-=81 -> byte3 bit 3 + sync
        new object[] { 0, 81, "Normal", (byte)0x00, (byte)0x00, (byte)0x13 },  // Y+=81 -> byte3 bit 4 + sync
        new object[] { 0, -81, "Normal", (byte)0x00, (byte)0x00, (byte)0x23 }, // Y-=81 -> byte3 bit 5 + sync
        new object[] { 121, 0, "Normal", (byte)0x05, (byte)0x05, (byte)0x07 }, // 1+9+27+81=121
        new object[] { -121, 0, "Normal", (byte)0x0A, (byte)0x0A, (byte)0x0B },
        new object[] { 0, 121, "Normal", (byte)0x50, (byte)0x50, (byte)0x13 },
        new object[] { 0, -121, "Normal", (byte)0xA0, (byte)0xA0, (byte)0x23 },
        new object[] { 121, -121, "Normal", (byte)0xA5, (byte)0xA5, (byte)0x27 },
        new object[] { -121, 121, "Normal", (byte)0x5A, (byte)0x5A, (byte)0x1B },

        // Jump (control = 0x80, byte3 = 0x83)
        new object[] { 1, 0, "Jump", (byte)0x01, (byte)0x00, (byte)0x83 },
        new object[] { 0, 1, "Jump", (byte)0x40, (byte)0x00, (byte)0x83 },
        new object[] { -1, 0, "Jump", (byte)0x02, (byte)0x00, (byte)0x83 },
        new object[] { 0, -1, "Jump", (byte)0x80, (byte)0x00, (byte)0x83 },

        // ColorChange/Stop (control = 0xC0, byte3 = 0xC3) - dx=0, dy=0 per spec
        new object[] { 0, 0, "ColorChange", (byte)0x00, (byte)0x00, (byte)0xC3 },

        // END marker (special - not via EncodeMovement)
        new object[] { 0, 0, "End", (byte)0xF3, (byte)0x00, (byte)0x00 },
    };

    [Theory]
    [MemberData(nameof(GoldenVectors))]
    public void GoldenVector_ReferenceMatch(int dx, int dy, string control, byte expectedB1, byte expectedB2, byte expectedB3)
    {
        byte controlByte = control switch
        {
            "Normal" => DstSpec.StitchNormal,
            "Jump" => DstSpec.StitchJump,
            "ColorChange" => DstSpec.StitchColorChange,
            "End" => DstSpec.StitchEnd,
            _ => throw new ArgumentException($"Unknown control: {control}")
        };

        (byte actualB1, byte actualB2, byte actualB3) actual;
        
        if (control == "End")
        {
            // END is special - use EncodeEnd()
            actual = DstMovementEncoder.EncodeEnd();
        }
        else
        {
            actual = DstMovementEncoder.EncodeMovement(dx, dy, controlByte);
        }

        actual.actualB1.Should().Be(expectedB1, $"Byte1 mismatch for ({dx},{dy}) {control}: expected 0x{expectedB1:X2}, got 0x{actual.actualB1:X2}");
        actual.actualB2.Should().Be(expectedB2, $"Byte2 mismatch for ({dx},{dy}) {control}: expected 0x{expectedB2:X2}, got 0x{actual.actualB2:X2}");
        actual.actualB3.Should().Be(expectedB3, $"Byte3 mismatch for ({dx},{dy}) {control}: expected 0x{expectedB3:X2}, got 0x{actual.actualB3:X2}");

        // Also verify round-trip for non-END records
        if (control != "End")
        {
            var (decX, decY, decCtrl) = DstMovementEncoder.DecodeMovement(actual.actualB1, actual.actualB2, actual.actualB3);
            decX.Should().Be(dx, $"Decoded X mismatch for ({dx},{dy}) {control}");
            decY.Should().Be(dy, $"Decoded Y mismatch for ({dx},{dy}) {control}");
            
            // Verify control bits (upper 2 bits of byte3) - decCtrl already has the 2-bit value
            byte expectedCtrlBits = control switch
            {
                "Normal" => 0,
                "Jump" => 2,       // 10 in bits 7-6
                "ColorChange" => 3, // 11 in bits 7-6
                _ => throw new ArgumentException()
            };
            decCtrl.Should().Be(expectedCtrlBits, $"Control bits mismatch for ({dx},{dy}) {control}");
        }
    }
    ...
    public void CommandBytes_IndependentVerification()
    {
        ...
        // END marker must use EncodeEnd() - EncodeMovement rejects StitchEnd
        var (eb1, eb2, eb3) = DstMovementEncoder.EncodeEnd();
        eb1.Should().Be(0xF3);
        eb2.Should().Be(0x00);
        eb3.Should().Be(0x00);
    ...
    [Fact]
    public void EncodeMovement_RejectsStitchEnd()
    {
        // EncodeMovement should throw when given StitchEnd (0xF0)
        var action = () => DstMovementEncoder.EncodeMovement(0, 0, DstSpec.StitchEnd);
        action.Should().Throw<ArgumentException>()
            .WithMessage("*StitchEnd*")
            .WithMessage("*EncodeEnd()*");
    }
