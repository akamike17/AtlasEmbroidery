...
/// <summary>
/// Round-trip semantic fidelity tests for EXP format
/// </summary>
public class ExpRoundTripTests
{
    ...
    public async Task RoundTrip_SimpleShape_PreservesStitches()
    {
        ...
        // EXP has no header, so name is not preserved
        readProject.Name.Should().Be("Untitled"); // Default name when read
    }
    ...
    public async Task RoundTrip_MultiPass_Stable()
    {
        ...
        // EXP has no header, so name is not preserved - always "Untitled"
        project3.Name.Should().Be("Untitled");
    }
    ...
    private AtlasProject CreateMultiColorProject()
    {
        ...
        {
            Name = "Blue",
            Vertices = new List<Point> { new Point(0, 1000), new Point(1000, 1000), new Point(500, 0) },
            IsClosed = false,
            StitchParams = StitchParams.DefaultFor(StitchType.Running)
        };
